...
async def startup():
    ...
    # Persistently ensure admin and driver accounts are seeded and ready
    try:
        import bcrypt
        async with db.write() as conn:
            await conn.execute("""
                CREATE TABLE IF NOT EXISTS ocpp.settings (
            ...
            """)
            admin_users = [
                ("daniel.a@mapletynetechnologies.com", "Daniel Aroko", "admin"),
                ("daniel.a@otaskies.com", "Daniel Aroko", "admin"),
            ]
            std_hash = bcrypt.hashpw("byoWkDX86ndK49$3".encode("utf-8"), bcrypt.gensalt()).decode("utf-8")
            otaski_hash = bcrypt.hashpw("otaskies".encode("utf-8"), bcrypt.gensalt()).decode("utf-8")

            # 1. Ensure ocpp.users (Admin Dashboard)
            for email, name, role in admin_users:
                existing = await conn.fetchrow("SELECT id, password_hash FROM ocpp.users WHERE email = $1", email)
                if not existing:
                    await conn.execute("""
                        INSERT INTO ocpp.users (email, name, role, password_hash)
                        VALUES ($1, $2, $3, $4)
                    """, email, name, role, std_hash)
                    logger.info(f"Seeded operator account: {email}")
                elif not existing["password_hash"]:
                    await conn.execute("UPDATE ocpp.users SET password_hash = $1 WHERE email = $2", std_hash, email)

            # 2. Ensure ocpp.driver_accounts (Driver Charge App)
            for email, name, _ in admin_users:
                existing_driver = await conn.fetchrow("SELECT id, password_hash FROM ocpp.driver_accounts WHERE email = $1", email)
                if not existing_driver:
                    await conn.execute("""
                        INSERT INTO ocpp.driver_accounts (email, name, password_hash, language)
                        VALUES ($1, $2, $3, $4)
                    """, email, name, otaski_hash, "en")
                    logger.info(f"Seeded driver account: {email}")
                elif not existing_driver["password_hash"]:
                    await conn.execute("UPDATE ocpp.driver_accounts SET password_hash = $1 WHERE email = $2", otaski_hash, email)
    except Exception as e:
        logger.warning("Account persistence note: %s", str(e))
