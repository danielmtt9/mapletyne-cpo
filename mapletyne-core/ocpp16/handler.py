...
class ChargePointHandler(ChargePointHelpersMixin):
    ...
    async def _on_status_notification(self, payload: dict) -> dict:
        ...
        if error_code != "NoError" or status == "Faulted":
            ...
            if not getattr(self, "_self_healing_active", False):
                import asyncio
                asyncio.create_task(self._trigger_self_healing(connector_id, error_code))
        else:
            logger.info(f"[{self.cp_id}] Connector {connector_id}: {status}")
        return {}

    async def _trigger_self_healing(self, connector_id: int, error_code: str):
        """Tiered automated self-healing sequence."""
        import asyncio
        self._self_healing_active = True
        try:
            logger.info(f"[{self.cp_id}] 🛠️ Initiating Tier 1 self-healing (UnlockConnector) on connector {connector_id}")
            await self.event_bus.publish(Event(
                type=EventType.ALERT, charge_point=self.cp_id, connector=connector_id,
                data={"type": "SELF_HEALING_TIER_1", "action": "UnlockConnector", "reason": error_code, "status": "Executing Unlock"}
            ))
            # Send unlock command if supported
            try:
                await self.call("UnlockConnector", {"connectorId": connector_id})
            except Exception:
                pass
            ...
            if state and state.get(f"connector_{connector_id}_status") == "Faulted":
                logger.info(f"[{self.cp_id}] 🛠️ Tier 1 unresolved. Initiating Tier 2 self-healing (Soft Reset)")
                await self.event_bus.publish(Event(
                    type=EventType.ALERT, charge_point=self.cp_id, connector=connector_id,
                    data={"type": "SELF_HEALING_TIER_2", "action": "SoftReset", "reason": error_code, "status": "Executing Soft Reset"}
                ))
                try:
                    await self.call("Reset", {"type": "Soft"})
                except Exception:
                    pass
        except Exception as e:
    ...
    async def _on_authorize(self, payload: dict) -> dict:
        ...
        if id_tag.startswith(("APP_", "SIM_", "TEST_", "EVEREST_")) or id_tag in ("REMOTE", "DEADBEEF", "04A1B2C3", "ISO15118", "BasicAuthSecureKey2026!"):
