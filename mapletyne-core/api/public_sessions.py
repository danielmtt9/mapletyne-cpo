...
class SessionCreate(BaseModel):
    ...
    driver_start_soc_pct: Optional[int] = None
    driver_target_soc_pct: Optional[int] = None
...
async def create_session(req: SessionCreate):
    ...
    from api.payments import create_stripe_checkout, STRIPE_SECRET_KEY

    checkout_url = f"/app/session/{session_id}"
    payment_status = "authorized"

    if STRIPE_SECRET_KEY:
        try:
            stripe_url = await create_stripe_checkout(
                session_id=session_id,
                cp_id=req.cp_id,
                connector_id=req.connector_id,
                amount_cents=2500,
                currency=currency.lower(),
                customer_email=req.driver_email
            )
            if stripe_url:
                checkout_url = stripe_url
                payment_status = "pending"
        except Exception as e:
            logger.warning("Stripe checkout creation failed (%s), falling back to direct auth", e)
        ...
        await conn.execute("""
            INSERT INTO ocpp.public_sessions
                (id, cp_id, connector_id, driver_phone, driver_email,
                 ...
                 driver_start_soc_pct, driver_target_soc_pct, battery_capacity_kwh, currency)
            VALUES ($1::uuid, $2, $3, $4, $5, $6, $7, NOW(),
                    $8, $9, $10, $11,
                    $12, $13, $14, $15)
        """, session_id, req.cp_id, req.connector_id,
             req.driver_phone, req.driver_email, rate_kwh, payment_status,
             pricing_tier, spot_price_at_start, cost_basis_at_start, rate_kwh,
             req.driver_start_soc_pct, req.driver_target_soc_pct, req.battery_capacity_kwh, currency)

    logger.info(
        "Session %s created for %s connector %d | tier=%s rate=%.4f status=%s currency=%s",
        session_id[:8], req.cp_id, req.connector_id, pricing_tier, rate_kwh, payment_status, currency,
    )

    # Immediately trigger OCPP RemoteStartTransaction if direct/authorized
    if payment_status == "authorized":
        id_tag = f"APP_{session_id[:8].upper()}"
        try:
            msg_id = await charger_registry.send_remote_start(req.cp_id, req.connector_id, id_tag)
            if msg_id:
                logger.info("RemoteStartTransaction sent: session=%s cp=%s connector=%d", session_id[:8], req.cp_id, req.connector_id)
            else:
                logger.warning("RemoteStart queued or charger not ready: %s", req.cp_id)
        except Exception as exc:
            logger.warning("Could not dispatch RemoteStart to %s: %s", req.cp_id, exc)

    # Fire-and-forget SMS with session link
    if req.driver_phone:
        charge_app_url = os.getenv("CHARGE_APP_URL", "https://opencpo.mapletyne.com/app")
...
async def get_session(session_id: str):
    ...
    async with db.read() as conn:
        row = await conn.fetchrow("""
            SELECT id::text AS id, cp_id, connector_id, driver_phone, driver_email,
                   ...
                   driver_start_soc_pct, driver_target_soc_pct, battery_capacity_kwh,
                   latest_soc_pct, unlocked_at, idle_duration_seconds, idle_fee_amount,
                   currency
              FROM ocpp.public_sessions
             WHERE id = $1::uuid
        """, session_id)
    ...
    result["currency"] = row["currency"] or "GBP"
    result["is_estimated_soc"] = False

    kwh_delivered = float(row["kwh_delivered"] or 0)
    current_kw = 0.0
    soc_pct = row["latest_soc_pct"]
    ...
    if row["ocpp_transaction_id"]:
        ...
        if ocpp_session:
            ...
            if live:
                current_kw = float(live.get("power_kw", 0))
                if live.get("soc_pct") is not None and float(live.get("soc_pct")) > 0:
                    soc_pct = float(live.get("soc_pct"))
                    result["is_estimated_soc"] = False
                if live.get("energy_kwh"):
                    kwh_delivered = float(live["energy_kwh"])

    # Fallback to manual estimation if SoC not provided natively by EV / AC charger
    if soc_pct is None and row["driver_start_soc_pct"] is not None and row["battery_capacity_kwh"]:
        try:
            start_soc = float(row["driver_start_soc_pct"])
            cap_kwh = float(row["battery_capacity_kwh"])
            if cap_kwh > 0:
                calc_soc = start_soc + (kwh_delivered / cap_kwh) * 100.0
                target = float(row["driver_target_soc_pct"] or 100)
                soc_pct = min(100.0, min(target, round(calc_soc, 1)))
                result["is_estimated_soc"] = True
        except Exception:
            pass

    result["current_kw"] = current_kw
    result["soc_pct"] = soc_pct
    result["kwh_delivered"] = kwh_delivered

    # Calculate total current cost
    rate = float(row["rate_kwh"] or 0.35)
    idle_fee = float(row["idle_fee_amount"] or 0.0)
    result["total_cost"] = round((kwh_delivered * rate) + idle_fee, 2)

    return result


@router.post("/sessions/{session_id}/unlock")
async def unlock_session_cable(session_id: str):
    """Trigger UnlockConnector for the active or finished session connector."""
    from state.charger_registry import send_unlock
    async with db.read() as conn:
        row = await conn.fetchrow("""
            SELECT id::text AS id, cp_id, connector_id
              FROM ocpp.public_sessions
             WHERE id = $1::uuid
        """, session_id)
    ...
    msg_id = await send_unlock(cp_id, connector_id)
    if msg_id is None:
        raise HTTPException(502, f"Charger {cp_id} failed to process unlock request")
    ...
    logger.info("Unlock requested for session %s on charger %s conn %d", session_id[:8], cp_id, connector_id)
    return {"session_id": session_id, "unlocked": True, "msg_id": msg_id}


class SocParamsUpdate(BaseModel):
    driver_start_soc_pct: Optional[int] = None
    driver_target_soc_pct: Optional[int] = None
    battery_capacity_kwh: Optional[float] = None


@router.post("/sessions/{session_id}/soc-params")
async def update_session_soc_params(session_id: str, req: SocParamsUpdate):
    """Update driver battery parameters for SoC estimation."""
    async with db.write() as conn:
        await conn.execute("""
            UPDATE ocpp.public_sessions
               SET driver_start_soc_pct = COALESCE($2, driver_start_soc_pct),
                   driver_target_soc_pct = COALESCE($3, driver_target_soc_pct),
                   battery_capacity_kwh = COALESCE($4, battery_capacity_kwh)
             WHERE id = $1::uuid
        """, session_id, req.driver_start_soc_pct, req.driver_target_soc_pct, req.battery_capacity_kwh)
