...
async def session_stats_today(
    site: Optional[str] = Query(None, description="Filter stats by site/location"),
):
    """Today's energy (kWh), session count, and revenue."""
    today = date_type.today().isoformat()
    conditions = [
        "s.simulated = FALSE",
        "s.start_time >= CURRENT_DATE",
        "s.start_time < CURRENT_DATE + INTERVAL '1 day'",
    ]
    params = []
    idx = 1
    if site and site.lower() != "all":
        conditions.append(f"(cp.site = ${idx} OR cp.site ILIKE ${idx} OR cp.metadata->>'city' ILIKE ${idx})")
        params.append(site)
        idx += 1
    ...
    async with db.read() as conn:
        row = await conn.fetchrow(f"""
            SELECT
                COALESCE(SUM(s.energy_kwh) FILTER (WHERE s.status = 'completed'), 0) AS energy_kwh,
                COUNT(*) AS session_count,
                COALESCE(SUM(s.energy_kwh * COALESCE((cp.metadata->>'tariff_kwh')::float, 0.42)) FILTER (WHERE s.status = 'completed'), 0) AS total_revenue
            FROM ocpp.sessions s
            LEFT JOIN ocpp.charge_points cp ON cp.id = s.charge_point
            WHERE {' AND '.join(conditions)}
        """, *params)
    energy = float(row["energy_kwh"]) if row and row["energy_kwh"] is not None else 0.0
    sessions = int(row["session_count"]) if row and row["session_count"] is not None else 0
    revenue = float(row["total_revenue"]) if row and row["total_revenue"] is not None else 0.0
    return {
        "energy_kwh": energy,
        "total_kwh": energy,
