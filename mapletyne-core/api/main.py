...
from api.fleet_bays import router as fleet_bays_router
from api.fleet_schedules import router as fleet_schedules_router
...
from api.ems import router as ems_router
...
app.include_router(ems_router, dependencies=_mgmt)
app.include_router(events_router, prefix="/api/v1/events", tags=["Events"])
app.include_router(users_router, prefix="/api/v1/users", tags=["Users"], dependencies=_mgmt)
app.include_router(vehicles_router, prefix="/api/v1/fleet/vehicles", tags=["Fleet Vehicles"], dependencies=_mgmt)
app.include_router(fleet_bays_router, prefix="/api/v1/fleet/depots", tags=["Fleet Depots"], dependencies=_mgmt)
app.include_router(fleet_schedules_router, prefix="/api/v1/fleet/schedules", tags=["Fleet Schedules"], dependencies=_mgmt)
...
# ── Static Uploads Mount (Logos, Icons, Branding) ──────────────────────────
from fastapi.staticfiles import StaticFiles
from pathlib import Path

uploads_path = Path(os.getenv("UPLOADS_DIR", "/var/lib/opencpo/uploads"))
try:
    uploads_path.mkdir(parents=True, exist_ok=True)
    app.mount("/static/uploads", StaticFiles(directory=str(uploads_path), check_dir=False), name="uploads")
except Exception as e:
    import logging
    logging.getLogger(__name__).warning(f"Could not mount static uploads: {e}")
