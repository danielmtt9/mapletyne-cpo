...
@router.post("/{cp_id}/unlock", dependencies=[Depends(management_auth)])
async def unlock_connector(cp_id: str, connector_id: int = Query(1)):
    """Send UnlockConnector command to a charger."""
...
@router.post("/{cp_id}/self-heal", dependencies=[Depends(management_auth)])
async def trigger_self_heal(cp_id: str, connector_id: int = Query(1)):
    """Trigger manual tiered self-healing sequence (Tier 1 Unlock + Tier 2 Soft Reset)."""
    from state.charger_registry import send_unlock, send_reset, is_connected

    if not is_connected(cp_id):
        raise HTTPException(400, f"Charger {cp_id} is not connected")

    tier1_id = await send_unlock(cp_id, connector_id)
    tier2_id = await send_reset(cp_id, "Soft")
    return {
        "status": "Accepted",
        "charge_point": cp_id,
        "connector_id": connector_id,
        "tier1_unlock_msg_id": tier1_id,
        "tier2_reset_msg_id": tier2_id,
        "message": "Tier 1 (Unlock) & Tier 2 (Soft Reset) executed successfully"
    }
...
async def create_charger(req: CreateChargerRequest):
    ...
    async with db.write() as conn:
        ...
        meta = {}
        for k in ("display_name", "address", "city", "latitude", "longitude", "max_power_kw", "tariff_kwh"):
        ...
        await conn.execute("""
            INSERT INTO ocpp.charge_points
                (id, vendor, model, serial_number, ocpp_version, site, simulated, metadata)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8::jsonb)
        """, req.id, req.vendor, req.model, req.serial_number,
            req.ocpp_version, req.site, req.simulated, json.dumps(meta))
