...
from state.settings import get_setting, put_setting
...
async def setup_org(body: StepOrgRequest):
    ...
    branding["company_name"] = body.name
    branding["portal_tagline"] = f"{body.name} Portal"
    await put_setting("branding", branding)
...
async def setup_branding(body: StepBrandingRequest):
    ...
    branding["portal_tagline"] = body.charge_app_name
    await put_setting("branding", branding)
