...
async def management_auth(
    ...
    if not provided:
        logger.warning(f"Management API: missing key path={request.url.path} from={request.client.host if request.client else '?'}")
        raise HTTPException(
            status_code=401,
    ...
    # 1. Check direct MANAGEMENT_API_KEY match
    if _MANAGEMENT_API_KEY and hmac.compare_digest(provided, _MANAGEMENT_API_KEY):
        return

    # 2. Check Admin JWT token (Keycloak RS256 or Local HS256)
    try:
        from api.admin_auth import verify_token
        payload = verify_token(provided)
        roles = payload.get("roles", [])
        role = payload.get("role", "")
        if (
            role in ("admin", "operator", "superadmin", "billing", "fleet", "user")
            or any(r in roles for r in ("cpo-admin", "site-operator", "billing-manager", "fleet-manager", "admin"))
        ):
            return
    except Exception:
        pass

    logger.warning(f"Management API: unauthorized request (bad key/token) path={request.url.path} from={request.client.host if request.client else '?'}")
    raise HTTPException(
        status_code=401,
