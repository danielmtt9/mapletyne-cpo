...
# ── Public Branding & Visual Tokens ─────────────────────────────────────

@router.get("/branding")
async def get_public_branding():
    """Return public visual branding tokens (no auth required)."""
    from state.settings import get_setting
    branding = await get_setting("branding")
    org = await get_setting("organization")
    logo_data_uri = branding.get("logo_data_uri", "")
    logo_url = branding.get("logo_url", "")
    favicon_url = branding.get("favicon_url") or logo_data_uri or logo_url or ""
    return {
        "company_name": branding.get("company_name", "") or org.get("company_name", ""),
        "app_title": branding.get("app_title", "Charge Station Management System"),
        "portal_tagline": branding.get("portal_tagline", "Enterprise EV Charging Management (CSMS)"),
        "logo_url": logo_url,
        "logo_data_uri": logo_data_uri,
        "favicon_url": favicon_url,
        "primary_color": branding.get("primary_color", "#4edea3"),
        "secondary_color": branding.get("secondary_color", "#1e293b"),
        "accent_color": branding.get("accent_color", "#3b82f6"),
        "bg_color": branding.get("bg_color", "#0b1326"),
        "card_color": branding.get("card_color", "#171f33"),
        "support_email": branding.get("support_email", "") or org.get("invoicing_email", ""),
        "legal_entity": branding.get("legal_entity", "") or org.get("legal_entity", ""),
        "vat_number": branding.get("vat_number", "") or org.get("tax_vat_id", ""),
...
async def get_public_charger(cp_id: str):
    """Return single charger info with live connector status."""
    async with db.read() as conn:
    ...
    live = await _get_charger_live(cp_id)
    connectors = []
    if live:
        for key, val in live.items():
            if key.startswith("connector_") and key.endswith("_status"):
                ...
                if conn_id > 0:
                    connectors.append({"id": conn_id, "type": "CCS2", "status": val})
    if not connectors:
        connectors.append({"id": 1, "type": "CCS2", "status": live.get("status", "Available") if live else "Available"})
    ...
    return {
        ...
        "power_kw": r["max_power_kw"] or 0,
        "connectors": connectors,
        "status": live.get("status", "Available") if live else (r["status"] or "Available"),
        "operator": os.getenv("OPERATOR_NAME", "Your CPO"),
        "tariff_kwh": energy_rate,
        "energy_rate": energy_rate,
        "time_rate": 0.00,
...
@router.post("/chargers/{cp_id}/connectors/{connector_id}/unlock")
async def unlock_charger_connector(cp_id: str, connector_id: int):
    """Emergency or manual unlock for a charger connector."""
    from state.charger_registry import send_unlock, is_connected
    if not is_connected(cp_id):
        logger.warning("UnlockConnector requested for disconnected charger %s", cp_id)
    msg_id = await send_unlock(cp_id, connector_id)
    if msg_id is None:
        raise HTTPException(502, f"Failed to send UnlockConnector to {cp_id}")
    return {"status": "Accepted", "charge_point": cp_id, "connector_id": connector_id, "msg_id": msg_id}
