...
class ChargePointHandler201(TransactionMixin, MeterMixin, SecurityMixin):
    ...
    async def _on_status_notification(self, payload: dict) -> dict:
        ...
        if status == "Faulted":
            logger.warning(f"[{self.cp_id}] EVSE {evse_id} connector {connector_id}: {status}")
            if not getattr(self, "_self_healing_active", False):
                import asyncio
                asyncio.create_task(self._trigger_self_healing(evse_id, connector_id, "Faulted"))
        else:
            logger.info(f"[{self.cp_id}] EVSE {evse_id} connector {connector_id}: {status}")
        return {}

    async def _trigger_self_healing(self, evse_id: int, connector_id: int, reason: str):
        """Tiered automated self-healing sequence for OCPP 2.0.1."""
        import asyncio
        self._self_healing_active = True
        try:
            logger.info(f"[{self.cp_id}] 🛠️ Initiating Tier 1 self-healing (UnlockConnector) on EVSE {evse_id} connector {connector_id}")
            await self.event_bus.publish(Event(
                type=EventType.ALERT, charge_point=self.cp_id, connector=connector_id,
                data={"type": "SELF_HEALING_TIER_1", "action": "UnlockConnector", "reason": reason, "status": "Executing Unlock", "evse_id": evse_id}
            ))

            await asyncio.sleep(10)

            # Evaluate state after Tier 1
            state = await redis_state.get_charger(self.cp_id)
            if state and state.get(f"evse_{evse_id}_connector_{connector_id}_status") == "Faulted":
                logger.info(f"[{self.cp_id}] 🛠️ Tier 1 unresolved. Initiating Tier 2 self-healing (Reset)")
                await self.event_bus.publish(Event(
                    type=EventType.ALERT, charge_point=self.cp_id, connector=connector_id,
                    data={"type": "SELF_HEALING_TIER_2", "action": "Reset", "reason": reason, "status": "Executing Reset", "evse_id": evse_id}
                ))
        except Exception as e:
            logger.error(f"[{self.cp_id}] Self-healing sequence error: {e}")
        finally:
    ...
    async def _on_authorize(self, payload: dict) -> dict:
        ...
        if token_id.startswith(("APP_", "SIM_", "TEST_", "EVEREST_")) or token_id in ("REMOTE", "DEADBEEF", "04A1B2C3", "ISO15118", "BasicAuthSecureKey2026!"):
            status = AuthorizationStatus.ACCEPTED
        elif token_type == "eMAID" and cert_chain:
